...
# Bot configuration
BOT_TOKEN= "8731822720:AAERTiz-7MalhPLVr_2Z0dJWWVKUU6AZM5g"
OWNER_ID= 6784382795
...
def access_record(user_id):
    conn = _access_db()
    row = conn.execute(
        "SELECT status, expires_at, requested_at FROM access_users WHERE user_id=?",
        (int(user_id),)
    ).fetchone()
    conn.close()
    if not row:
        return None
    return {"status": row[0], "expires_at": row[1], "requested_at": row[2]}

def has_access(user_id):
    if int(user_id) in ADMIN_IDS:
        return True
    rec = access_record(user_id)
    if not rec or rec["status"] != "approved" or not rec["expires_at"]:
        return False
    try:
    ...
    if datetime.now() >= expires:
        conn = _access_db()
        conn.execute("UPDATE access_users SET status='expired' WHERE user_id=?", (int(user_id),))
        conn.commit()
        conn.close()
        return False
    return True
...
def request_access(message):
    uid = message.chat.id
    rec = access_record(uid)
    if rec and rec["status"] == "pending":
        bot.send_message(uid, "⏳ Your access request is still pending owner approval.")
        return
    if rec and rec["status"] == "approved" and has_access(uid):
        open_authorized_bot(message)
        return

    now = datetime.now().isoformat(timespec="seconds")
    conn = _access_db()
    conn.execute(
        "INSERT INTO access_users(user_id,status,expires_at,requested_at) VALUES(?,?,?,?) "
        "ON CONFLICT(user_id) DO UPDATE SET status=excluded.status, expires_at=NULL, requested_at=excluded.requested_at",
        (uid, "pending", None, now)
    )
    conn.commit()
    conn.close()
    ...
    markup.row(
        types.InlineKeyboardButton("✅ ACCEPT", callback_data=f"access_accept:{uid}"),
        types.InlineKeyboardButton("❌ DECLINE", callback_data=f"access_decline:{uid}")
    )
    bot.send_message(
        OWNER_ID,
    ...
    bot.send_message(uid, "📨 *Access request sent to the owner.*\nPlease wait for approval.", parse_mode="Markdown")

def approval_expiry_keyboard(uid):
    m = types.InlineKeyboardMarkup(row_width=2)
    m.row(
        types.InlineKeyboardButton("1 Day", callback_data=f"access_expiry:{uid}:1d"),
        types.InlineKeyboardButton("7 Days", callback_data=f"access_expiry:{uid}:7d")
    )
    m.row(
        types.InlineKeyboardButton("30 Days", callback_data=f"access_expiry:{uid}:30d"),
        types.InlineKeyboardButton("Permanent", callback_data=f"access_expiry:{uid}:36500d")
    )
    m.row(types.InlineKeyboardButton("✍️ CUSTOM DATE", callback_data=f"access_custom:{uid}"))
    return m

def grant_access(uid, duration):
    expires = datetime.now() + duration
    conn = _access_db()
    conn.execute(
        "UPDATE access_users SET status='approved', expires_at=? WHERE user_id=?",
        (expires.isoformat(timespec="seconds"), int(uid))
    )
    conn.commit()
    conn.close()
    return expires

def open_authorized_bot(message):
    ...
    markup = create_main_keyboard(chat_id)
    bot.send_message(
        chat_id,
        "🏎️ *CPM DASHBOARD BOT*\n━━━━━━━━━━━━━━━━\n✅ Access approved. Choose your CPM version:",
        reply_markup=markup,
        parse_mode="Markdown"
    )

def set_custom_expiry_step(message, uid):
    if message.chat.id != OWNER_ID:
        return
    try:
        value = message.text.strip()
        # Accept either: YYYY-MM-DD HH:MM or YYYY-MM-DD
        if len(value) == 10:
            expires = datetime.strptime(value, "%Y-%m-%d")
        else:
            expires = datetime.strptime(value, "%Y-%m-%d %H:%M")
        ...
        conn.commit()
        conn.close()
        bot.send_message(OWNER_ID, f"✅ Access approved for `{uid}` until `{expires.strftime('%Y-%m-%d %H:%M')}`", parse_mode="Markdown")
        bot.send_message(uid, f"✅ *Your access was approved!*\n📅 Expires: `{expires.strftime('%Y-%m-%d %H:%M')}`\n\nSend /start to open the bot.", parse_mode="Markdown")
    except Exception:
        bot.send_message(OWNER_ID, "❌ Invalid date. Send again as:\n`2026-12-31 23:59`\nor\n`2026-12-31`", parse_mode="Markdown")
        bot.register_next_step_handler(message, set_custom_expiry_step, uid)
ALLOWED_KEYS = ["NELHUMBLE CPM TOOL"]
CHANNEL_ID = "-1003948068860"
CHANNEL_LINK = "https://t.me/cpm1giveawayhuby"
...
def format_account_info(info: Dict[str, Any], cars_owned: int = 0) -> str:
    if not info.get("ok"):
        return "❌ **Cannot load account data**"
    return f"""🏎️ **CPM DASHBOARD**
━━━━━━━━━━━━━━━━
👤 Name: `{info.get('name', 'Unknown')}`
🆔 ID: `{info.get('localID', 'Unknown')}`
💵 Money: `{int(info.get('money', 0)):,}`
🪙 Coins: `{int(info.get('coin', 0)):,}`
...
def create_cpm1_keyboard(chat_id):
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.row(types.InlineKeyboardButton("👤 ACCOUNT", callback_data="dash_account"),
               types.InlineKeyboardButton("💰 ECONOMY", callback_data="dash_economy"))
    markup.row(types.InlineKeyboardButton("🚗 VEHICLES", callback_data="dash_vehicles"),
               types.InlineKeyboardButton("🔓 UNLOCKS", callback_data="dash_unlocks"))
    markup.row(types.InlineKeyboardButton("🔄 REFRESH", callback_data="refresh_account"),
               types.InlineKeyboardButton("🚪 LOGOUT", callback_data="logout"))
    return markup

def create_dashboard_section_keyboard(section):
    m = types.InlineKeyboardMarkup(row_width=2)
    if section == "account":
        m.row(types.InlineKeyboardButton("✏️ SET NAME", callback_data="dash_set_name"), types.InlineKeyboardButton("🆔 SET ID", callback_data="cpm1_change_id"))
        m.row(types.InlineKeyboardButton("📧 CHANGE GMAIL", callback_data="cpm1_change_email"), types.InlineKeyboardButton("🔑 CHANGE PASSWORD", callback_data="cpm1_change_pass"))
        m.row(types.InlineKeyboardButton("📋 CLONE ACCOUNT", callback_data="cpm1_clone"))
    elif section == "economy":
        m.row(types.InlineKeyboardButton("💵 MONEY", callback_data="cpm1_money"), types.InlineKeyboardButton("💵 50M", callback_data="dash_money_50m"))
        m.row(types.InlineKeyboardButton("✍️ CUSTOM MONEY", callback_data="cpm1_money"))
        m.row(types.InlineKeyboardButton("🪙 COIN", callback_data="cpm1_coin"), types.InlineKeyboardButton("🪙 500K", callback_data="dash_coin_500k"))
        m.row(types.InlineKeyboardButton("✍️ CUSTOM COIN", callback_data="cpm1_coin"))
        m.row(types.InlineKeyboardButton("🏁 RACE WIN", callback_data="dash_race_win"), types.InlineKeyboardButton("🏁 RACE LOSE", callback_data="dash_race_lose"))
        m.row(types.InlineKeyboardButton("👑 KING RANK", callback_data="cpm1_rank_advanced"))
    elif section == "vehicles":
        m.row(types.InlineKeyboardButton("📋 CLONE ALL CARS", callback_data="cpm1_clone"))
        m.row(types.InlineKeyboardButton("🚗 UNLOCK ALL CARS", callback_data="cpm1_unlock_cars"))
    elif section == "unlocks":
        m.row(types.InlineKeyboardButton("⚡ W16", callback_data="cpm1_w16"), types.InlineKeyboardButton("💨 SMOKE", callback_data="cpm1_smoke"))
        m.row(types.InlineKeyboardButton("⛽ FUEL", callback_data="cpm1_fuel"), types.InlineKeyboardButton("📯 HORN", callback_data="cpm1_horns"))
        m.row(types.InlineKeyboardButton("🏠 ALL PAID HOUSE", callback_data="cpm1_unlock_houses"), types.InlineKeyboardButton("🚨 SIREN", callback_data="dash_siren"))
        m.row(types.InlineKeyboardButton("👩 FEMALE EQUIPMENT", callback_data="cpm1_unlock_equip_female"))
        m.row(types.InlineKeyboardButton("👨 MALE EQUIPMENT", callback_data="cpm1_unlock_equip_male"))
        m.row(types.InlineKeyboardButton("🎭 ANIMATION", callback_data="cpm1_unlock_animations"), types.InlineKeyboardButton("💡 HEADLIGHTS", callback_data="dash_headlights"))
        m.row(types.InlineKeyboardButton("🏆 ALL LEVELS", callback_data="cpm1_complete_levels"))
    m.row(types.InlineKeyboardButton("🔙 BACK", callback_data="dash_home"))
    return m
...
def show_cpm1_menu(chat_id, message=None, force_refresh=False):
    ...
    try:
        ...
        if td and td.get('auth_token'):
            cars = cpm1_get_cars(td['auth_token'])
            cars_owned = len(cars) if isinstance(cars, list) else 0
    except Exception:
        cars_owned = 0
    full_text = format_account_info(info, cars_owned)
...
def start(message):
    ...
    if has_access(chat_id):
        open_authorized_bot(message)
        return
    rec = access_record(chat_id)
    if rec and rec["status"] == "pending":
        bot.send_message(chat_id, "⏳ *Your access request is pending owner approval.*", parse_mode="Markdown")
        return
    if rec and rec["status"] == "expired":
        bot.send_message(chat_id, "⌛ *Your access has expired.*\nSend /start again to request new access.", parse_mode="Markdown")
        return
    markup = create_start_keyboard(chat_id)
    bot.send_message(
        chat_id,
        "🔒 *ACCESS REQUIRED*\n━━━━━━━━━━━━━━━━\nSend a request to the owner. The owner can accept, decline, and choose your custom expiry date.",
        reply_markup=markup,
        parse_mode="Markdown"
    )
...
def handle_callback(call):
    ...
    if data == "request_access":
        request_access(call.message)
        return

    if data.startswith("access_accept:"):
        if chat_id != OWNER_ID:
            bot.answer_callback_query(call.id, "Owner only.", show_alert=True)
            return
        uid = int(data.split(":")[1])
        bot.edit_message_text(
            f"✅ Request accepted for `{uid}`. Now choose the expiry:",
            chat_id,
            call.message.message_id,
            reply_markup=approval_expiry_keyboard(uid),
            parse_mode="Markdown"
        )
        return

    if data.startswith("access_decline:"):
        if chat_id != OWNER_ID:
            bot.answer_callback_query(call.id, "Owner only.", show_alert=True)
            return
        uid = int(data.split(":")[1])
        conn = _access_db()
        conn.execute("UPDATE access_users SET status='declined', expires_at=NULL WHERE user_id=?", (uid,))
        conn.commit()
        conn.close()
        bot.edit_message_text(f"❌ Request declined for `{uid}`.", chat_id, call.message.message_id, parse_mode="Markdown")
        bot.send_message(uid, "❌ *Your access request was declined by the owner.*", parse_mode="Markdown")
        return

    if data.startswith("access_expiry:"):
        if chat_id != OWNER_ID:
            bot.answer_callback_query(call.id, "Owner only.", show_alert=True)
            return
        _, _, uid_s, choice = data.split(":")
        uid = int(uid_s)
        days = int(choice[:-1])
        expires = grant_access(uid, timedelta(days=days))
        bot.edit_message_text(
            f"✅ Access approved for `{uid}` until `{expires.strftime('%Y-%m-%d %H:%M')}`",
            chat_id,
            call.message.message_id,
            parse_mode="Markdown"
        )
    ...
    if data.startswith("access_custom:"):
        if chat_id != OWNER_ID:
            bot.answer_callback_query(call.id, "Owner only.", show_alert=True)
            return
        uid = int(data.split(":")[1])
        bot.send_message(chat_id, f"✍️ Send custom expiry for `{uid}`:\\n`YYYY-MM-DD HH:MM`\\nExample: `2026-12-31 23:59`", parse_mode="Markdown")
        bot.register_next_step_handler(call.message, set_custom_expiry_step, uid)
        return
    ...
    if data not in ["request_access"] and not data.startswith("access_"):
        if not has_access(chat_id):
            bot.answer_callback_query(call.id, "Access required. Send /start.", show_alert=True)
            return
    ...
    if data == "dash_home":
        show_cpm1_menu(chat_id)
        return
    if data == "dash_account":
        bot.send_message(chat_id, "👤 **ACCOUNT**\n━━━━━━━━━━━━━━━━\nChoose an action:", reply_markup=create_dashboard_section_keyboard("account"), parse_mode='Markdown')
        return
    if data == "dash_economy":
        bot.send_message(chat_id, "💰 **ECONOMY**\n━━━━━━━━━━━━━━━━\nChoose an action:", reply_markup=create_dashboard_section_keyboard("economy"), parse_mode='Markdown')
        return
    if data == "dash_vehicles":
        bot.send_message(chat_id, "🚗 **VEHICLES**\n━━━━━━━━━━━━━━━━\nChoose an action:", reply_markup=create_dashboard_section_keyboard("vehicles"), parse_mode='Markdown')
        return
    if data == "dash_unlocks":
        bot.send_message(chat_id, "🔓 **UNLOCKS**\n━━━━━━━━━━━━━━━━\nChoose an action:", reply_markup=create_dashboard_section_keyboard("unlocks"), parse_mode='Markdown')
        return

    # ====== CPM1 ======
    web_uid = get_web_uid(chat_id)

    if data == "dash_set_name":
        bot.send_message(chat_id, "👤 **Enter new name:**", parse_mode='Markdown')
        user_states[chat_id] = {'awaiting_set_name': True}
        return
    if data == "dash_money_50m":
        execute_result = run_async(nuker.set_money(web_uid, 50000000))
        bot.send_message(chat_id, "✅ **50M money saved!**" if execute_result.get('ok') else f"❌ {execute_result.get('message')}", parse_mode='Markdown')
        return
    if data == "dash_coin_500k":
        execute_result = run_async(nuker.set_coin(web_uid, 500000))
        bot.send_message(chat_id, "✅ **500K coins saved!**" if execute_result.get('ok') else f"❌ {execute_result.get('message')}", parse_mode='Markdown')
        return
    if data == "dash_race_win":
        bot.send_message(chat_id, "🏁 **Enter race wins amount:**", parse_mode='Markdown')
        user_states[chat_id] = {'awaiting_race_win': True}
        return
    if data == "dash_race_lose":
        bot.send_message(chat_id, "🏁 **Enter race loses amount:**", parse_mode='Markdown')
        user_states[chat_id] = {'awaiting_race_lose': True}
        return
    if data == "dash_siren":
        async def _siren():
            await nuker.load(web_uid)
            td=nuker.get_token_data(web_uid); email=td.get('email') if td else None
            d=deepcopy(nuker.get_record(web_uid,email)); d['boughtPoliceSirens']=list(range(1,101))
            return await nuker._save(web_uid,d,force_fields={'boughtPoliceSirens'})
        r=run_async(_siren()); bot.send_message(chat_id, "✅ **Siren unlocked!**" if r.get('ok') else f"❌ {r.get('message')}", parse_mode='Markdown'); return
    if data == "dash_headlights":
        async def _lights():
            await nuker.load(web_uid)
            td=nuker.get_token_data(web_uid); email=td.get('email') if td else None
            d=deepcopy(nuker.get_record(web_uid,email)); d['boughtPoliceLights']=list(range(1,101))
            return await nuker._save(web_uid,d,force_fields={'boughtPoliceLights'})
        r=run_async(_lights()); bot.send_message(chat_id, "✅ **Headlights unlocked!**" if r.get('ok') else f"❌ {r.get('message')}", parse_mode='Markdown'); return
...
def handle_all_messages(message):
    ...
    if chat_id in user_states:
        ...
        if state.get('awaiting_set_name'):
            name = text.strip()
            if not name:
                bot.send_message(chat_id, "❌ Name cannot be empty!", parse_mode='Markdown'); return
            web_uid = user_sessions[chat_id].get('web_uid')
            result = run_async(nuker.set_player_name(web_uid, name))
            bot.send_message(chat_id, "✅ **Name saved!**" if result.get('ok') else f"❌ {result.get('message')}", parse_mode='Markdown')
            del user_states[chat_id]; show_cpm1_menu(chat_id); return
        if state.get('awaiting_race_win'):
            try: amount = int(text.strip().replace(',',''))
            except: bot.send_message(chat_id,"❌ Enter a valid number!",parse_mode='Markdown'); return
            web_uid=user_sessions[chat_id].get('web_uid'); result=run_async(nuker._set_floats(web_uid, [(8,float(amount))]))
            bot.send_message(chat_id,"✅ **Race wins saved!**" if result.get('ok') else f"❌ {result.get('message')}",parse_mode='Markdown')
            del user_states[chat_id]; show_cpm1_menu(chat_id); return
        if state.get('awaiting_race_lose'):
            try: amount = int(text.strip().replace(',',''))
            except: bot.send_message(chat_id,"❌ Enter a valid number!",parse_mode='Markdown'); return
            web_uid=user_sessions[chat_id].get('web_uid'); result=run_async(nuker._set_floats(web_uid, [(9,float(amount))]))
            bot.send_message(chat_id,"✅ **Race loses saved!**" if result.get('ok') else f"❌ {result.get('message')}",parse_mode='Markdown')
            del user_states[chat_id]; show_cpm1_menu(chat_id); return
...
if __name__ == "__main__":
    print("="*60)
    print("☠️☠️☠️ NELHUMBLE CPM TOOL BOT - CPM1 + CPM2 ULTIMATE ☠️☠️☠️")
    print("="*60)
    print("✅ Bot is running!")
    print("👑 Admins:7634875658,6784382795 ")
    print("🔑 Keys: CPM - NELHUMBLE To0L ")
